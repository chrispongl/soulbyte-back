...
export const handlePayRent: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    try {
        ...
        if (cityId) {
            const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId } });
            if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
        }

        if (!spouseId) {
            if (useQueue) {
                const fees = calculateFees(rentWei, cityFeeBps);
            ...
            if (payA.greaterThan(0)) {
                ...
                if (useQueue) {
                    const fees = calculateFees(payAWei, cityFeeBps);
                    rentFees = {
            ...
            if (payB.greaterThan(0)) {
                ...
                if (useQueue) {
                    const fees = calculateFees(payBWei, cityFeeBps);
                    rentFees = {
                        platformFee: rentFees.platformFee + fees.platformFee,
                        cityFee: rentFees.cityFee + fees.cityFee
                    };
...
export const handleBuyItem: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    try {
        ...
        if (isCityGenesisSale) {
            ...
            if (useQueue) {
                const fees = calculateFees(costWei, cityFeeBps);
...
export const handleBuyFromStore: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    try {
        if (useQueue) {
            let cityFeeBps = FEE_CONFIG.CITY_FEE_DEFAULT_BPS;
            const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: store.cityId } });
            if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
            const fees = calculateFees(priceWei, cityFeeBps);
