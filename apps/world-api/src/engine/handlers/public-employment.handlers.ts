...
export const handleCollectSalary: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    const grossSalary = new Decimal(employment.dailySalarySbyte.toString()).mul(daysToPayFor);
    const platformFee = grossSalary.mul(FEE_CONFIG.PLATFORM_FEE_BPS).div(10000);
    const netSalary = grossSalary.sub(platformFee);
    ...
    try {
        ...
        const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: cityVault.cityId } });
        if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
        const feeBreakdown = calculateFees(salaryWei, cityFeeBps);
    ...
    const platformFeeAmount = new Decimal(ethers.formatEther(salaryFees.platformFee));
    const cityFeeAmount = new Decimal(ethers.formatEther(salaryFees.cityFee));
    stateUpdates.push({
        table: 'transaction',
        operation: 'create',
        data: {
