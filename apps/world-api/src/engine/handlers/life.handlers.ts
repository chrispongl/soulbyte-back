...
const FORAGE_ENERGY_COST = 10;
const FORAGE_HUNGER_GAIN = 15;
    ...
    return {
        stateUpdates: [{
            ...
            data: {
                ...
                energy: Math.min(100, (agentState.energy ?? 0) + 5),
                health: Math.min(100, (agentState.health ?? 0) + 5)
            }
        }],
        events: [{
            actorId: actor.id,
...
export const handleForage: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    return fail(actor.id, EventType.EVENT_FORAGED, 'Foraging disabled: use restaurants or buy food');
};
