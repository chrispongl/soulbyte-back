...
async function processActorIntents(
    ...
    if (actor.agentState) {
        ...
        if (isBusy && !isIntentAllowedWhileBusy(intent.type) && !ownerOverride && !emergencyBusyOverride) {
            await markIntentBlocked(intent.id, `Busy (${activityState}) until tick ${activityEndTick}`, currentTick);
            return { processedCount, events };
        }

        if (activityState === 'RESTING' && isBusy && !isIntentAllowedWhileBusy(intent.type) && !emergencyBusyOverride) {
            await markIntentBlocked(intent.id, 'Cannot act while resting', currentTick);
            return { processedCount, events };
        }
        ...
        try {
            ...
            if (agentState.cityId) {
                const cityPolicy = await prisma.cityPolicy.findUnique({
                    where: { cityId: agentState.cityId },
                });
                if (cityPolicy) {
                    cityFeeBps = Number(cityPolicy.cityFeeBps);
                }
            }
            const fees = calculateFees(wageWei, cityFeeBps);
