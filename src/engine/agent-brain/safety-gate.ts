...
function getIntentCost(decision: IntentDecision, ctx: AgentContext): number {
    ...
    if (decision.intentType === 'INTENT_FOUND_BUSINESS') {
        const config = BUSINESS_CONFIG[decision.params.businessType];
        if (!config) return 0;
        return config.buildCost + config.minCapital;
    }
...
function validateIntent(decision: IntentDecision, ctx: AgentContext): string | null {
    ...
    if (type === 'INTENT_PROPOSE_ALLIANCE' || type === 'INTENT_PROPOSE_DATING' || type === 'INTENT_ARREST' || type === 'INTENT_STEAL' || type === 'INTENT_ASSAULT' || type === 'INTENT_FRAUD') {
