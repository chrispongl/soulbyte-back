...
export class HousingDomain {
    static getCandidates(ctx: AgentContext): CandidateIntent[] {
        ...
        if (ownedVacantHome && ctx.housing.currentRental?.id !== ownedVacantHome.id) {
            const ownedBase = homeless ? 95 : ctx.housing.currentRental ? 65 : 40;
            candidates.push({
                ...
                reason: homeless
                    ...
                    : `Moving into owned home to avoid rent`,
                domain: 'housing',
            });
        }
        ...
        if (rental) {
            ...
            candidates.push({
                ...
                basePriority: (homeless ? 85 : 35) + scarcityBoost + dealBoost,
                personalityBoost: PersonalityWeights.getBoost(ctx.personality.energyManagement, true),
                reason: homeless
                    ? `Securing housing before work`
                    : `Upgrading housing with affordable rent`,
                domain: 'housing',
            });
        }
        ...
        if (purchase) {
            ...
            const buyBase = homeless ? 70 : 30;
            candidates.push({
                intentType: IntentType.INTENT_BUY_PROPERTY,
                params: { propertyId: purchase.id, maxPrice: purchase.salePrice },
                basePriority: buyBase + scarcityBoost + valueBoost,
                personalityBoost: buyBoost,
                reason: homeless
                    ? `Buying a home to avoid homelessness`
                    : `Buying property as a long-term plan`,
                domain: 'housing',
            });
        }
        ...
        if (emptyLotForSale) {
            candidates.push({
                ...
                basePriority: (homeless ? 60 : 25) + scarcityBoost,
                personalityBoost: PersonalityWeights.getBoost(ctx.personality.riskTolerance, true),
                reason: homeless
                    ? `Buying an empty lot to build a home`
                    : `Buying an empty lot for future housing`,
                domain: 'housing',
            });
        }
        ...
        if (buildableLot) {
            ...
            if (buildingType && buildCost > 0 && ctx.state.balanceSbyte >= buildCost * 0.2) {
                candidates.push({
                    ...
                    basePriority: (homeless ? 75 : 35) + scarcityBoost,
                    personalityBoost: PersonalityWeights.getBoost(ctx.personality.patience, true),
                    reason: homeless
                        ? `Building housing on owned lot`
                        : `Building property on owned lot`,
                    domain: 'housing',
                });
            }
        }
