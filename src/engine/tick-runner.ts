...
import { computeEconomicSnapshots, computeGodEconomicReport, pruneOldSnapshots } from '../services/economy-snapshot.service.js';
...
async function clearExpiredActivities(currentTick: number): Promise<void> {
    ...
    for (const state of expired) {
        ...
        if (state.activityState === 'RESTING') {
            ...
            const baseEnergyGain = restHours * 10;
            const baseHealthGain = Math.max(2, Math.floor(restHours));
...
async function applyRestingRecovery(currentTick: number): Promise<void> {
    ...
    for (const state of resting) {
        ...
        const energyInterval = Math.max(1, Math.round(10 / energyDivisor));
        const healthInterval = Math.max(1, Math.round(30 / healthDivisor));
            ...
            try {
                ...
                if (decision.intentType !== 'INTENT_IDLE') {
                    const existingPending = await prisma.intent.findFirst({
                        where: { actorId: agent.id, status: { in: ['pending', 'queued'] } },
