...
import { calculateFees, FEE_CONFIG } from '../../config/fees.js';
...
const BUSINESS_CONFIG: Record<string, { minWealth: string; buildCost: number; employeesL1: number }> = {
    ...
    ENTERTAINMENT: { minWealth: 'W4', buildCost: 6000, employeesL1: 2 },
};
...
const BUSINESS_MIN_CAPITAL: Record<string, { sbyte: number; mon: number }> = {
    ...
    ENTERTAINMENT: { sbyte: 2000, mon: 5 },
};
...
export const handleFoundBusiness: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    if (!params?.businessType || !params.cityId || !params.landId) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Missing params');
    }
    if (actor.frozen) return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Agent frozen');
    const jail = await prisma.jail.findUnique({ where: { actorId: actor.id } });
    if (jail && jail.releaseTick > tick) return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Agent jailed');
    if (!agentState) return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Missing agent state');

    const config = BUSINESS_CONFIG[params.businessType];
    if (!config) return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Invalid business type');
    if (!meetsWealthRequirement(agentState.wealthTier || 'W0', config.minWealth)) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, `Wealth tier too low (${config.minWealth})`);
    }
    const fullActor = await prisma.actor.findUnique({ where: { id: actor.id } });
    if (Number(fullActor?.reputation || 0) < 150) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Reputation too low');
    }

    const existing = await prisma.business.findFirst({
        where: { ownerId: actor.id, businessType: params.businessType }
    });
    if (existing) return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Already owns this business type');

    const property = await prisma.property.findUnique({ where: { id: params.landId } });
    if (!property || property.cityId !== params.cityId) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Invalid land');
    }
    if (property.underConstruction) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Property under construction');
    }
    ...
    if (isHouseConversion && property.tenantId && property.tenantId !== actor.id) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'House is occupied');
    }
    ...
    if (needsPurchase) {
        ...
        if (purchaseResult.intentStatus !== IntentStatus.EXECUTED) {
            ...
            return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, reason);
        }
    ...
    if (isHouseConversion) {
        if (!ownsProperty && !purchased) {
            return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Must own the house to convert');
        }
    } else if (!ownsProperty && !rentsProperty && !purchased) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Must own or rent lot');
    }

    if (!wallet) return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'No wallet');
    const agentWallet = await prisma.agentWallet.findUnique({ where: { actorId: actor.id } });
    if (!agentWallet) return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'No on-chain wallet');
    ...
    const totalFeeBps = FEE_CONFIG.PLATFORM_FEE_BPS + cityFeeBps;
    ...
    if (balance.lessThan(totalRequired)) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Insufficient funds');
    }
    if (new Decimal(agentWallet.balanceMon.toString()).lessThan(minCap.mon)) {
        return fail(actor.id, EventType.EVENT_BUSINESS_FOUNDED, 'Insufficient MON for business gas');
    }
    ...
    const businessName = params.proposedName || `${params.businessType} ${businessId.slice(0, 4)}`;
    ...
    if (useQueue) {
        ...
        const fees = calculateFees(injectWei, cityFeeBps);
    ...
    return {
        ...
        events: purchaseEvents.concat([{
            actorId: actor.id,
            type: EventType.EVENT_BUSINESS_FOUNDED,
...
export const handleSetPrices: IntentHandler = async (intent, actor) => {
    ...
    if (params.pricePerService !== undefined) config.pricePerService = params.pricePerService;
...
export const handleBuyBusiness: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    const cityFeeBps = Number(cityPolicy?.cityFeeBps ?? FEE_CONFIG.CITY_FEE_DEFAULT_BPS);
    const cityFeeRate = new Decimal(cityFeeBps).div(10000);
    const platformFeeRate = new Decimal(FEE_CONFIG.PLATFORM_FEE_BPS).div(10000);
...
export const handleDissolveBusiness: IntentHandler = async (intent, actor) => {
    ...
    return {
        stateUpdates: [
            { table: 'business', operation: 'update', where: { id: business.id }, data: { status: 'DISSOLVED', dissolvedTick: tick, isOpen: false, treasury: 0 } },
...
export const handleInjectBusinessFunds: IntentHandler = async (intent, actor, agentState, wallet) => {
    ...
    const config = (business.config || {}) as Record<string, any>;
    const finance = config.finance || { totalInjected: 0, totalWithdrawn: 0 };
    finance.totalInjected = Number(finance.totalInjected || 0) + netInjected;
    config.finance = finance;

    return {
        stateUpdates: [
            { table: 'business', operation: 'update', where: { id: business.id }, data: { treasury: { increment: netInjected }, config } },
...
export const handleVisitBusiness: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    if (useQueue) {
        ...
        const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: business.cityId } });
        if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
        const fees = calculateFees(ethers.parseEther(price.toString()), cityFeeBps);
    ...
    return {
        stateUpdates: [
            {
                ...
                operation: 'update',
                where: { id: business.id },
                data: { treasury: { increment: netAmount }, customerVisitsToday: { increment: 1 } }
            },
            {
                table: 'businessWallet',
                operation: 'update',
        ...
        events: [{
            ...
            targetIds: [business.id],
            outcome: EventOutcome.SUCCESS,
            sideEffects: { price: price.toString(), txHash, effects, queued: useQueue }
        }],
        intentStatus: useQueue ? IntentStatus.QUEUED : IntentStatus.EXECUTED
    };
};
...
export const handleBuyStoreItem: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    if (useQueue) {
        ...
        const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: business.cityId } });
        if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
        const fees = calculateFees(ethers.parseEther(totalPrice.toString()), cityFeeBps);
        netAmount = Number(ethers.formatEther(fees.netAmount));
        feePlatform = Number(ethers.formatEther(fees.platformFee));
        feeCity = Number(ethers.formatEther(fees.cityFee));
        const job = createOnchainJobUpdate({
