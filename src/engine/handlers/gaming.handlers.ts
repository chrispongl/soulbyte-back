...
export const handleChallengeGame: IntentHandler = async (intent, actor, agentState, wallet, tick) => {
    ...
    const stake = Math.max(GAMING_CONFIG.MIN_STAKE, Number(params.stake ?? GAMING_CONFIG.MIN_STAKE));
    ...
    if (target.agentState?.cityId !== agentState.cityId) {
        return fail(actor.id, EventType.EVENT_GAME_CHALLENGE, 'Target not in same city');
    }
    ...
    try {
        ...
        if (useQueue) {
            ...
            if (agentState.cityId) {
                const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: agentState.cityId } });
                if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
            }
            const fees = calculateFees(stakeWei, cityFeeBps);
...
export const handleAcceptGame: IntentHandler = async (intent, actor, agentState, wallet, tick, seed) => {
    ...
    if (!challenger.agentState || challenger.agentState.cityId !== agentState.cityId) {
        return fail(actor.id, EventType.EVENT_GAME_RESULT, 'Challenger not in same city');
    }
    ...
    const platformFee = new Decimal(stake).mul(2).mul(FEE_CONFIG.PLATFORM_FEE_BPS).div(10000);
    ...
    try {
        ...
        if (useQueue) {
            if (!escrowed) {
                let cityFeeBps = FEE_CONFIG.CITY_FEE_DEFAULT_BPS;
                if (challengerState?.cityId) {
                    const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: challengerState.cityId } });
                    if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
                }
                const fees = calculateFees(stakeWei, cityFeeBps);
                ...
                const job = createOnchainJobUpdate({
                    ...
                    payload: {
                        ...
                        cityId: challengerState?.cityId ?? null,
                        toAddressOverride: CONTRACTS.PLATFORM_FEE_VAULT,
                    },
            ...
            let accepterCityFeeBps = FEE_CONFIG.CITY_FEE_DEFAULT_BPS;
            if (accepterState?.cityId) {
                const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: accepterState.cityId } });
                if (cityPolicy) accepterCityFeeBps = Number(cityPolicy.cityFeeBps);
            }
            const accepterFees = calculateFees(stakeWei, accepterCityFeeBps);
            ...
            const accepterJob = createOnchainJobUpdate({
                ...
                payload: {
                    ...
                    amountWei: stakeWei.toString(),
                    reason: 'gaming_pvp_stake',
                    cityId: accepterState?.cityId ?? null,
                    toAddressOverride: CONTRACTS.PLATFORM_FEE_VAULT,
                },
                actorId: actor.id,
                relatedIntentId: intent.id,
            ...
            if (!escrowed) {
                challengerStakeTx = await agentTransferService.transfer(
                    ...
                    stakeWei,
                    'gaming_pvp_stake',
                    challengerState?.cityId || undefined,
                    CONTRACTS.PLATFORM_FEE_VAULT
                );
            }
            accepterStakeTx = await agentTransferService.transfer(
                ...
                stakeWei,
                'gaming_pvp_stake',
                accepterState?.cityId || undefined,
                CONTRACTS.PLATFORM_FEE_VAULT
            );
        }
        ...
        if (useQueue) {
            let payoutCityFeeBps = FEE_CONFIG.CITY_FEE_DEFAULT_BPS;
            if (agentState.cityId) {
                const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: agentState.cityId } });
                if (cityPolicy) payoutCityFeeBps = Number(cityPolicy.cityFeeBps);
            }
            const payoutFees = calculateFees(payoutWei, payoutCityFeeBps);
    ...
    if (escrowed && stake > 0) {
        ...
        try {
            ...
            if (useQueue) {
                let cityFeeBps = FEE_CONFIG.CITY_FEE_DEFAULT_BPS;
                if (challenge.cityId) {
                    const cityPolicy = await prisma.cityPolicy.findUnique({ where: { cityId: challenge.cityId } });
                    if (cityPolicy) cityFeeBps = Number(cityPolicy.cityFeeBps);
                }
                const fees = calculateFees(stakeWei, cityFeeBps);
...
export const handlePlayGame: IntentHandler = async (intent, actor, agentState, wallet, tick, seed) => {
    ...
    const actorWins = actorScore >= houseScore;
    ...
    try {
        ...
        if (useQueue) {
            ...
            if (actorWins) {
                const payoutWei = ethers.parseEther(winnings.toString());
                const payoutFees = calculateFees(payoutWei, cityFeeBps);
                payoutTx = { txHash: null, platformFee: payoutFees.platformFee, cityFee: payoutFees.cityFee };
                const payoutJob = createOnchainJobUpdate({
                    jobType: 'AGENT_TRANSFER_SBYTE',
                    payload: {
...
export const handleBet: IntentHandler = async (intent, actor, agentState, wallet, _tick, seed) => {
    ...
    const betAmount = Math.max(GAMING_CONFIG.MIN_STAKE, Number(params?.betAmount ?? GAMING_CONFIG.MIN_STAKE));
    ...
    const platformFee = new Decimal(betAmount).mul(FEE_CONFIG.PLATFORM_FEE_BPS).div(10000);
    const odds = 2.5;
    ...
    try {
        ...
        if (useQueue) {
            ...
            const fees = calculateFees(betWei, cityFeeBps);
