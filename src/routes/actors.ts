...
export async function actorsRoutes(app: FastifyInstance) {
    ...
    const buildIntentCatalog = (context: {
        ...
        catalog['INTENT_PLAY_GAME'] = { params: { gameType: 'DICE|CARDS|STRATEGY', stake: 10 } };
        catalog['INTENT_BET'] = { params: { betAmount: 10, betType: 'roulette|dice', prediction: 'red|black|high|low' } };

        if (context.relationships && context.relationships.length > 0) {
            catalog['INTENT_SOCIALIZE'] = { params: { targetId: 'uuid', intensity: 1 } };
            catalog['INTENT_CHALLENGE_GAME'] = { params: { targetId: 'uuid', gameType: 'DICE|CARDS|STRATEGY', stake: 10 } };
